...
namespace MVC_Voorbeeld3.Services
{
    public class PersoonService
    {
        ...
        {
            {
                1, new Persoon{ID=1, Voornaam="Jesse", Familienaam="James", Score=5, Wedde=1000, Geslacht = Geslacht.Man,
                            Paswoord="123", Geboren=new DateTime(1966,1,1), Gehuwd=true, Opmerkingen="Schurk van het Wilde Westen"}
            },
            {
            ...
            },
            {
                4, new Persoon {ID=4, Voornaam="Sarah",Familienaam="Bernhardt", Score=3, Wedde=4000, Paswoord="123",
                                Geboren =new DateTime(1966,4,4), Gehuwd=false, Opmerkingen="Rosine Bernardt", Geslacht=Geslacht.Vrouw}
            }
        };
        ...
        {
            return (from x in personen.Values where x.Wedde >= van && x.Wedde <= tot orderby x.Wedde select x).ToList();
        }
